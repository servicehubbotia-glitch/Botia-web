...
Layer WebViews — 10 files: sugar.html, sweetener.html, texture.html,
flavor.html, enumbers.html, animal_origin.html, woman_safefood.html,
halal.html, haram.html, mashbooh.html.
...
Every WebView has exactly the same shape and nothing more:

brand header · layer icon · title · one-paragraph lead ·
"Found in this label" · ingredient buttons · "Tap an ingredient" ·
two "More about" links · one-line disclaimer

No menu, no chat, no footer, no BOTIA CAN/CANNOT, no sources block,
no hreflang, no JSON-LD. They carry noindex.

Two exceptions:

halal.html shows no ingredients. It is the result of nothing being flagged.

woman_safefood.html handles both cases in one page: it shows ingredients
when ?trigger= is present, and a context message when it is not. That logic
lives in the page, not in the app, so the app never needs recompiling for it.
...
i18n/glossary.json → ingredient_names

the 14 ingredient_index.json

triggers-master.json
...
Create ingredients/{slug}.html — copy gelatine.html exactly

Create i18n/en/{slug}.json — copy gelatine.json exactly

Translate to the 13 remaining languages

Add it to the 14 ingredient_index.json

Regenerate the glossary, the triggers, the sitemap and master_kb.json

Upload master_kb.json to the Cloudflare KV
